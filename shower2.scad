...
// right wall
function right_wall_rough_width() = left_wall_rough_width() + drywall_thickness();
...
//    accent_x_offset=accent_x_rough_offset-(wall_prep_thickness()+tile_thickness()+2*grout_width());
    ...
    right_side_wall_width=right_wall_rough_width()+wall_prep_thickness()+tile_thickness()-(wall_prep_thickness()+tile_thickness()+grout_width());
    ...
    translate([0, 0, knee_rough_height()+wall_prep_thickness()-grout_width()]) {
        translate([0, -back_wall_shelf_rough_depth()-wall_prep_thickness(), 0]) {
            ...
            left_lower_side_wall_width=left_wall_rough_width()-knee_rough_thickness()-2*(wall_prep_thickness()+tile_thickness()+grout_width());

            // lower left wall
            translate([wall_prep_thickness(), -(tile_thickness()+grout_width()), 0]) {
                rotate([0,0,90]) tile_grid(width=-left_lower_side_wall_width,height=-lower_surround_height,first_tile_width=left_wall_first_tile_width()-back_wall_shelf_rough_depth());
            }
            ...
            if (accent_tile_vertical) {
                ...
                // lower right wall
                translate([back_wall_rough_width()-wall_prep_thickness(), -(tile_thickness()+grout_width()), 0]) {
                    rotate([0,0,-90]) tile_grid(width=right_lower_side_wall_width,height=-lower_surround_height,first_tile_width=right_wall_first_tile_width()-back_wall_shelf_rough_depth());
                }
            }
        }

        endcap_width=tile_thickness()+wall_prep_thickness()+knee_rough_thickness()+wall_prep_thickness()+tile_thickness();
        ...
        translate([wall_prep_thickness(), -left_wall_rough_width()-wall_prep_thickness(), 0]) {
            tile_grid(width=left_wall_knee_rough_width()-grout_width(),height=-lower_surround_height);
        }
        translate([left_wall_knee_rough_width()+wall_prep_thickness(), -left_wall_rough_width()-wall_prep_thickness()-tile_thickness(), 0]) {
            rotate([0,0,90]) tile_grid(width=endcap_width,height=-lower_surround_height);
        }
        translate([wall_prep_thickness()+tile_thickness()+grout_width(), -left_wall_rough_width()-wall_prep_thickness()-tile_thickness(), grout_width()]) {
            width=left_wall_knee_rough_width()-grout_width();
    ...
    translate([0, -wall_prep_thickness(), knee_rough_height()+wall_prep_thickness()]) {
        ...
        left_upper_side_wall_width=left_wall_rough_width()+wall_prep_thickness()+tile_thickness()-(wall_prep_thickness()+tile_thickness()+grout_width());
        ...
        if (accent_tile_vertical) { // vertical accent
            ...
            // upper back wall
            translate([wall_prep_thickness(), 0, accent_width()+grout_width()]) {
                tile_grid(width=back_wall_width,height=upper_surround_height-accent_width()-grout_width(),first_tile_width=back_wall_first_tile_width());
                translate([0,0,-grout_width()]) rotate([0,90,0]) accent_tile(width=accent_width(),length=back_wall_width,seed=8481);
            }

            // upper left wall
            translate([wall_prep_thickness(), -(tile_thickness()+grout_width()), accent_width()+grout_width()]) {
                rotate([0,0,90]) tile_grid(width=-left_upper_side_wall_width,height=upper_surround_height-accent_width()-grout_width(),first_tile_width=left_wall_first_tile_width());
                translate([0,-left_upper_side_wall_width,-grout_width()]) rotate([0,90,90]) accent_tile(width=accent_width(),length=left_upper_side_wall_width,seed=25);
            }

            // upper right wall
            translate([back_wall_rough_width()-wall_prep_thickness(), -(tile_thickness()+grout_width()), accent_width()+grout_width()]) {
                rotate([0,0,-90]) tile_grid(width=right_side_wall_width,height=upper_surround_height-accent_width()-grout_width(),first_tile_width=right_wall_first_tile_width());
                translate([0,0,-grout_width()]) rotate([0,90,-90]) accent_tile(width=accent_width(),length=right_side_wall_width,seed=188);
            }
        }
    }
        ...
        color([0.66,0.7,1.0],drywall_alpha) union() {
            translate([0,-left_wall_rough_width(),0]) {
                ...
                translate([0,-drywall_thickness(),0]) {
                    translate([drywall_thickness(),0,0]) cube([left_wall_knee_rough_width()-drywall_thickness(),drywall_thickness(),knee_rough_height()+drywall_thickness()]);
                    translate([-feet(1)-inches(5.5),0,0]) {
